            ...
            <span v-if="childComponent['type'] == 'checkbox'">
                ...
                <tr v-for="entry in filteredData">
                    <td v-for="key in columns">
                        <a v-if="recursive == true" @click="getChildren(entry.id, entry)">{{ entry[key] }}</a>
            ...
            capitalize: function (str) {
                if (str) {
                    if (typeof str !== 'object')
                    {
                        ...
                        {
                            return str.label.charAt(0).toUpperCase() + str.slice(1)
                        }
                        return str.key
                    }
                }
            ...
            getTotalRecord: function(id) {
                ...
                axios.get(url)
                    .then(response =>
                    {
                        ...
                        eventBus.$emit('setTotalRecord',response.data);
                    })
                    .catch(error => this.onGetDataFail(error.response.data));
            },
            getChildren: function (id, row={}) {
                if (row.hasOwnProperty('nama') && row.nama.length > 0)
                {
                    this.$emit('input', row);
                }
                this.setScroll(1);
